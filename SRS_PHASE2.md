...
`CONTEXT_CAPTURE`는 사용자가 특정 체크사항 화면에서 직접 자료를 생성한 경우 사용한다.
...
필요 시 Material에 선택적 Context 필드를 추가할 수 있다.
...
단, 정규 관계의 기준은 EvidenceMapping으로 유지한다.
...
강제 종료 가능하며 Closeout Snapshot을 저장하고 ResearchPlan을 CLOSED로 전환한다.
...
기존 `/api/materials/upload`를 재사용하되 client에서 저장 완료 후 Mapping API를 호출하거나, 선택적으로 orchestration endpoint를 둔다.
...
기존 전체 Evidence 관리 화면으로 유지한다. 부스 화면에서 생성한 자료도 동일 Material로 표시하고 연결된 Target/Checkpoint를 보여준다.
...
보고서 생성 전 Coverage, 미확인 P1/P2, Closeout 상태를 표시한다.
...
- 신규 모델 추가 방식으로 마이그레이션
- 기존 FieldDay/Material/Report ID 유지
- 기존 프로젝트는 ResearchPlan 없이도 정상 동작
- Plan 없는 프로젝트의 기존 Quick Analysis/Final Report 유지
...
- 저장된 Material이 해당 Checkpoint와 연결된다.
- 자료 추가 후 Coverage가 갱신된다.
- P1/P2 미충족 항목이 Gap으로 표시된다.
- Next Action이 최소 rule 기반으로 생성된다.
- Booth Closeout에서 빠진 항목을 확인할 수 있다.
- 전체 Field Closeout에서 미방문 부스와 핵심 미충족을 확인할 수 있다.
- Quick Analysis가 활성 Research Plan Context를 사용할 수 있다.
- Final Report가 Coverage와 미확인 사항을 활용할 수 있다.
...
10. Coverage 0% → 40% → 70% 갱신 확인
11. Gap/Next Action 확인
12. Booth Closeout 실행
13. B/C 부스 반복
14. Field Closeout
15. Final Report 생성
```
...
```text
2A. Prisma 모델 + Plan CRUD
2B. 조사 준비 UI + Target/Booth/Checkpoint 편집
2C. Booth Execution 화면
2D. Checkpoint In-context Capture
...
이 흐름이 iPhone에서 끊기지 않고 수행되면 2차 개발을 완료로 판단한다.
